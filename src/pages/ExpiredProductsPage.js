...
import '../styles/ExpiredProductsPage.css';  
...
const ExpiredProductsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const expired = products.filter((p) => isExpired(p.EXPIRATION_DATE));
  ...
  // ✅ Search filter
  const filteredProducts = expiredProducts.filter((p) => {
  ...
  if (loading) return <div>Loading expired products...</div>;
